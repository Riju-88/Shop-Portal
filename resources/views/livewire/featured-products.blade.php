<div>
    <div x-data="{
        ...
        <button @click="prev" class="absolute left-0 top-1/2 transform -translate-y-1/2 z-10 bg-gray-700 text-white p-2 rounded-full">
            <x-filament::icon icon="heroicon-m-arrow-left" class="h-5 w-5 text-white dark:text-gray-400" />
        </button>
                                    ...
                                    @if (!empty($product->image) && is_array($product->image) && isset($product->image[0]))
                                        @if (filter_var($product->image[0], FILTER_VALIDATE_URL))
                                            <a href="{{ route('product.detail', ['productId' => $product->id]) }}">
                                                <img src="{{ $product->image[0] }}" alt="{{ $product->name }}" class="h-full w-full object-cover object-center l">
                                            </a>
                                        @else
                                            <a href="{{ route('product.detail', ['productId' => $product->id]) }}">
                                                <img src="{{ asset('storage/' . $product->image[0]) }}" alt="{{ $product->name }}" class="h-full w-full object-cover object-center ">
                                            </a>
                                        @endif
                                    @else
        ...
        <button @click="next" class="absolute right-0 top-1/2 transform -translate-y-1/2 z-10 bg-gray-700 text-white p-2 rounded-full">
            <x-filament::icon icon="heroicon-m-arrow-right" class="h-5 w-5 text-white dark:text-gray-400" />
        </button>
    </div>
</div>
